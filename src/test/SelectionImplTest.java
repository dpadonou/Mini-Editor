...
class SelectionImplTest {
    ...
    StringBuilder testBf = new StringBuilder("Buffer créer pour faire des test sur les getters et getters setters.");
    SelectionImpl sp = new SelectionImpl(2, 12, testBf);

    @Test
    @DisplayName("Begin index should be lower than end index")
    void beginIndexShouldBeLowerThanEndIndex() {
        Exception exception = Assertions.assertThrows(IllegalArgumentException.class, () -> {
            new SelectionImpl(27, 8, testBf);
        });
    ...
    void EndIndexShouldBeGreaterThan0() {
        Exception exception = Assertions.assertThrows(IllegalArgumentException.class, () -> {
            new SelectionImpl(0, 0, testBf);
        });
        String msg = "L'index de fin doit être supérieure à 0.";
        assert (msg.equals(exception.getMessage()));
    }
